...
discover_modem_web_interfaces() {
    ...
    for ip in "${web_ips[@]}"; do
        ...
        if ping -c 1 -W 1 "$ip" >/dev/null 2>&1; then
            if curl -s --connect-timeout 2 "http://$ip" 2>/dev/null | grep -qi "huawei\|mobile.*wifi\|lte\|4g"; then
                ...
                local interface=""
                for iface in $(ip link show | grep -E "enx|usb" | cut -d: -f2 | tr -d ' '); do
...
# Функция диагностики одного модема
...
# Функция оптимизации одного модема
...
main() {
    ...
    if ! discover_modem_web_interfaces; then
        echo -e "${YELLOW}⚠️  Веб-интерфейсы не найдены, но USB модемы обнаружены${NC}"
        echo "Попробуйте:"
        echo "1. Настроить модемы с помощью универсального скрипта настройки"
        echo "2. Проверить, что SIM-карты активны"
        echo "3. Убедиться, что модемы в режиме HiLink"
        exit 1
    fi
