...
class DedicatedProxyServer:
    ...
    async def start(self):
        """Запуск с сырым TCP обработчиком для CONNECT"""
        if self._running:
        ...
        try:
            ...
            self._running = True
            logger.info(f"✅ RAW TCP proxy server started on port {self.port}")

        except Exception as e:
            logger.error(f"❌ Failed to start RAW TCP proxy server: {e}")
            self._running = False
            raise
    ...
    async def handle_raw_connection(self, reader, writer):
        ...
        try:
            ...
            logger.info(f"🔌 New raw connection from {client_addr}")
    ...
    async def handle_raw_connect(self, reader, writer, request_info):
        ...
        try:
            ...
            if not device or device.get('status') != 'online':
                await self.send_http_error(writer, 503, "Device not available")
                return
            ...
            try:
                ...
                target_sock.close()
                await self.send_http_error(writer, 502, "Connection failed")
                return
            ...
            logger.info(f"🚀 Starting pure TCP tunnel")
            ...
            try:
                await self.send_http_error(writer, 502, "Bad Gateway")
            except:
                pass
    ...
    async def run_pure_tcp_tunnel_raw(self, reader, writer, target_sock, host, port):
        ...
        try:
            logger.info(f"🔄 Starting PURE TCP tunnel: client <-> {host}:{port}")
            ...
            async def forward_client_to_target():
                ...
                try:
                    ...
                    logger.info(f"✅ Client->Target finished: {total_bytes} bytes")
            ...
            async def forward_target_to_client():
                ...
                try:
                    ...
                    logger.info(f"✅ Target->Client finished: {total_bytes} bytes")
            ...
            try:
                ...
                logger.info(f"🔚 Pure TCP tunnel completed: {host}:{port}")
            ...
            logger.info(f"🏁 Pure TCP tunnel ended: {host}:{port}")
    ...
    async def stop(self):
        ...
        try:
            ...
            self._running = False
            logger.info(f"RAW TCP proxy server stopped for {self.device_id}")

        except Exception as e:
            logger.error(f"Error stopping RAW TCP server: {e}")
